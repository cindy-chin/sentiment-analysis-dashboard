  ...
  if (!topic) {
    return (
      <Box textAlign="center" py={20}>
        <Heading size="md" mb={4}>Topic not found</Heading>
  ...
  return (
    <Box>
      ...
      <HStack gap={3} mb={2}>
        ...
        <Heading size="lg" bgGradient="to-r" gradientFrom="blue.600" gradientTo="green.500" bgClip="text">{topic.label}</Heading>
        <Badge colorPalette="green" fontSize="sm">{topic.postCount.toLocaleString()} posts</Badge>
      </HStack>
      <Text color="gray.600" mb={6}>
        Deep dive into this topic — engagement stats, keywords, word cloud, and top posts from r/Singapore.
      </Text>
      ...
      <SimpleGrid columns={{ base: 1, lg: 2 }} gap={6} mb={8}>
        ...
        <Card.Root bg="white" borderWidth="1px" borderColor="blue.50">
          <CardHeader pb={2}>
            <Heading size="sm" color="blue.700">Top Keywords (TF-IDF)</Heading>
          </CardHeader>
          <CardBody pt={0}>
      ...
      <VStack gap={4} align="stretch" mb={8}>
        {topic.posts.map((post) => (
          <Card.Root key={post.id} borderWidth="1px" borderColor="blue.100" bg="white" _hover={{ shadow: 'sm', borderColor: 'green.200' }} transition="all 0.2s">
            <CardBody>
